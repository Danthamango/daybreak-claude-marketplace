#!/usr/bin/env node
// Generates an operations dashboard (single self-contained HTML file) for any
// Billing partner account in Salesforce. Sibling to account-dashboard — same
// account hierarchy resolution, but surfaces the specific patients that need
// action: OOP call, pre-auth release, in-manufacturing follow-up, dental
// letter, impression kit chasing, bad impressions.
//
// Usage:
...
function printHelp() {
  ...
  --out       Output HTML path. Defaults to output/account-dashboard-ops/<slug>.html
...
function fetchContacts(locationIds) {
  ...
  const fields = [
    ...
    'Days_Since_MAD_Manufacturing_Started__c',
    'MAD_Shipped_Date__c',
    'MAD_Delivered_Date__c',
    'Absence_TMJD_Perio__c',
    'Daybreak_Impression_Kit_Ordered__c',
    'IK_Delivered_Date__c',
    'IK_Return_Delivered_Date__c',
    'Bad_Impression_Date__c',
  ].join(', ');
  const all = [];
  const chunkSize = 150;
...
function main() {
  ...
  const patients = contacts.map((c) => {
    ...
    return {
      ...
      daysMfg:
        c.Days_Since_MAD_Manufacturing_Started__c === null ||
        c.Days_Since_MAD_Manufacturing_Started__c === undefined
          ? null
          : Number(c.Days_Since_MAD_Manufacturing_Started__c),
      shipped: c.MAD_Shipped_Date__c || null,
      delivered: c.MAD_Delivered_Date__c || null,
      dentalLetter: c.Absence_TMJD_Perio__c === true,
      ikOrdered: c.Daybreak_Impression_Kit_Ordered__c === true,
      ikDelivered: c.IK_Delivered_Date__c || null,
      ikReturned: c.IK_Return_Delivered_Date__c || null,
      badImpressionDate: c.Bad_Impression_Date__c || null,
    };
  });
  ...
  const outPath = args.out
    ...
    : path.resolve(process.cwd(), 'output', 'account-dashboard-ops', slugify(root.Name) + '.html');
  ...
  const counts = {
    unactivated3d: patients.filter((p) => p.submitted && !p.activated && (Date.now() - new Date(p.submitted).getTime()) > 3 * 86400000).length,
    needsOOP: patients.filter((p) => p.activated && !p.ordered).length,
    preAuthPending: patients.filter((p) => p.ordered && !p.preAuthReleased).length,
    inMfg: patients.filter((p) => p.daysMfg !== null && !p.delivered).length,
    badImpressions: patients.filter((p) => p.badImpressionDate).length,
  };
  console.log(`Backlog: unactivated>3d=${counts.unactivated3d}  needs-OOP=${counts.needsOOP}  pre-auth=${counts.preAuthPending}  in-mfg=${counts.inMfg}  bad-imp=${counts.badImpressions}`);
}
